...
const CurrencyRatesButton = ({symbol,rate,openCurrencyConversion}) => (
    <div className="col-6 col-md-4 col-lg-2 mb-3">
        ...
        <button onClick={e => openCurrencyConversion(symbol,rate)} className="btn btn-outline-dark w-100 mt-3">
            Compare
        </button>
...
const CurrencyRatesTable = ({baseCurrencies,filterField,handleChange,openCurrencyConversion}) => {
    ...
    baseCurrencies = baseCurrencies.filter(currency => currency.symbol.includes(filterField.toUpperCase())).map(currency => {
        ...
        return <CurrencyRatesButton symbol={symbol} rate={rate} key={symbol} openCurrencyConversion={openCurrencyConversion} />
    });
