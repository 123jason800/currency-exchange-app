...
const CurrencyPair = ({rateYesterday,base,rate,currency}) => (
    <div className="font-weight-bold col-lg-3 col-md-4 col-12 col-sm-6">
        <div className="card currency-card shadow  align-items-center p-3 mb-3">
            <div className={`currency-flag-${base.toLowerCase()} currency-flag  currency-flag-xl  border border-dark`}></div> 
            <div className="my-2">
                1 {base} 
            </div>
            <div className="my-2">
                <span> {rateYesterday > rate ? <FontAwesomeIcon className="mr-2" size="lg" icon={faArrowDown} color="red"/> : <FontAwesomeIcon className="mr-2" size="lg" icon={faArrowUp} color="green"/>}</span>
